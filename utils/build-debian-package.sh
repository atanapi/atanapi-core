...
PACKAGE_VERSION=$($SRC_PATH/src/atanapid --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/atanapid $DEB_BIN
cp $SRC_PATH/src/atanapi-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/atanapi-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/atanapid.1 $DEB_MAN
cp $SRC_DOC/man/atanapi-cli.1 $DEB_MAN
cp $SRC_DOC/man/atanapi-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/atanapid.bash-completion $DEB_CMP/atanapid
cp $SRC_PATH/contrib/atanapi-cli.bash-completion $DEB_CMP/atanapi-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/atanapid.1
gzip --best -n $DEB_MAN/atanapi-cli.1
gzip --best -n $DEB_MAN/atanapi-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/atanapid $DEB_BIN/atanapi-cli
