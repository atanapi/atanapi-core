...
# atanapi_hash
RUN git clone https://github.com/atanapi/atanapi_hash
RUN cd atanapi_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} atanapi
RUN useradd -u ${USER_ID} -g atanapi -s /bin/bash -m -d /atanapi atanapi
...
RUN mkdir /atanapi-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /atanapi-src && \
