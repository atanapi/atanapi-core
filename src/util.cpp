...
const char * const BITCOIN_CONF_FILENAME = "atanapi.conf";
const char * const BITCOIN_PID_FILENAME = "atanapid.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "atanapid -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AtanapiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AtanapiCore
    // Mac: ~/Library/Application Support/AtanapiCore
    // Unix: ~/.atanapicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AtanapiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AtanapiCore";
#else
    // Unix
    return pathRet / ".atanapicore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty atanapi.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Atanapi Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Atanapi Core developers";
    }
