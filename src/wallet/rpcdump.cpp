...
UniValue removeaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(address)) {
        if (!(IsMine(*pwallet, address) & ISMINE_WATCH_ONLY)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Atanapi address - must be watch-only");
        }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The atanapi address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to atanapid)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    UniValue obj(UniValue::VOBJ);
    obj.pushKV("atanapicoreversion", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see atanapid log for details) and could "
