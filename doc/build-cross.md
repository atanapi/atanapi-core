Cross-compiliation of Atanapi Core
===============================

Atanapi Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building Atanapi.
...
When building Atanapi Core, use
...
Acquire the source in the usual way:
    ...
    git clone https://github.com/Raptor3um/atanapi
    cd atanapi
...
Note that for WSL the Atanapi Core source path MUST be somewhere in the default mount file system, for
example /usr/src/atanapi, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
It's not possible to build the Atanapi Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Atanapi Core source code).
