...
- [Development guidelines](#development-guidelines)
    - [General Atanapi Core](#general-atanapi-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Atanapi Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Raptor3um/atanapi/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/atanapid -printtoconsole
```
...
Atanapi Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Atanapi Core, where everyone uses
...
pay attention to for reviewers of Atanapi Core code.

General Atanapi Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Atanapi Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Atanapi Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `atanapi-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `atanapi-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Atanapi Core, the wallet was always
