...
*Setup instructions for a Gitian build of Atanapi Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Atanapi
...
- [Building Atanapi Core](#building-atanapi-core)
...
Clone the git repositories for Atanapi Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/atanapi/atanapi
...
in the Atanapi Core repository under 'Fetch and create inputs' to install sources which require
...
To build Atanapi Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Atanapi Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/atanapi/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/atanapi/atanapi
...
For example:
```bash
URL=https://github.com/crowning-/atanapi.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit atanapi=${COMMIT} --url atanapi=${URL} ../atanapi/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit atanapi=${COMMIT} --url atanapi=${URL} ../atanapi/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit atanapi=${COMMIT} --url atanapi=${URL} ../atanapi/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the atanapi git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../atanapi/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/atanapi
SIGPATH=/some/root/path/atanapi-detached-sigs

./bin/gbuild --url atanapi=${BTCPATH},signature=${SIGPATH} ../atanapi/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/atanapi-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/atanapi-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/atanapi-osx-build.assert
```
...
[atanapi/gitian.sigs](https://github.com/atanapi/gitian.sigs/) repository, or if that's not possible to create a pull
request.
